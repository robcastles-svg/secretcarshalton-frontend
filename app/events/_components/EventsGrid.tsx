...
import { getFeaturedImage, parseEventDate, stripHtml, type WPScEvent, type WPScEventTag } from "@/lib/wordpress";
...
export function EventsGrid({ events, tags }: { events: WPScEvent[]; tags: WPScEventTag[] }) {
        ...
        {visible.map((event) => {
          ...
          return (
            <li key={event.id}>
              ...
              <EventTopics topics={topics} className="event-card-topics" />
            </li>
          );
        })}
      </ul>
